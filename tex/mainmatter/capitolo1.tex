    ...
    \section{JavaFX}\label{sec:jfx}
        ...
        \subsection{Struttura di una Applicazione JavaFX}\label{sub:jfxStruttura}
            ...
            \begin{figure}[htbp]\label{fig:jfxLife}
                \centering
                \includegraphics{img/jfxLifecycle}
                \caption{La figura rappresenta il ciclo di vita di una applicazione JavaFX}
            \end{figure}

            La classe principale per una applicazione JavaFX deve estendere da \texttt{javafx.application.Application}. Il metodo \texttt{start()} è il punto di ingresso principale: lanciando una JavaFX Application attraverso il metodo \texttt{launch()} della classe di supporto \texttt{javafx.application.Platform}, verrà inizializzato il framework e poi verranno chiamati in ordine i metodi \texttt{init()} e \texttt{start(javafx.stage.Stage)}, che rappresentano di fatto il ciclo di inizializzazione ideale dell'applicazione.

            % TODO aggiungi analisi di Layout, Pannelli, Componenti e Stile
