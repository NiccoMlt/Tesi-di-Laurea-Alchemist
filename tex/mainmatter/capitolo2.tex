...
\chapter{Contributo}\label{ch:contributo}
    ...
    \section{Analisi dei requisiti}\label{sec:analisi}
        ...
        \subsection{Requisiti non funzionali}\label{sub:nonFunzionali}
            ...
            \subsubsection{JavaFX}\label{subsub:jfx}
                Come specificato nella sezione~\cref{sec:motivi}, il processo di sviluppo deve coinvolgere la libreria JavaFX come framework per la costruzione dell'interfaccia.
    ...
    \section{Design dell'interfaccia}\label{sec:design}
        ...
        Come è possibile vedere dalla \figurename~\cref{fig:mock:mainWindow}, si è scelto di adottare un'interfaccia composta da uno spazio disegnabile centrale, al quale viene sovrapposta nella parte inferiore una barra contenente dei controlli che permettono un'interazione semplice e diretta con le funzionalità di base:
        ...
        \begin{description}
          ...
          Nelle Figure~\crefrange{fig:mock:groups}{fig:mock:properties} è possibile osservare i diversi livelli del drawer laterale degli effetti.
    ...
    \section{Progettazione}\label{sec:progettazione}
        ...
        \subsection{La struttura dei drawer e le proprietà osservabili}\label{sub:drawer}
            Una volta realizzato il mockup illustrato nella Sezione~\ref{sec:design}, l'attenzione si è spostata su progettarne la struttura a livello software.
            Trascurando i dettagli implementativi prettamente legati all'implementazione grafica, di cui si parla nella Sezione~\cref{sec:dettagli}, è stato importante progettare la gestione degli eventi di modifica di proprietà di effetti e gruppi.
            Come detto nella Sottosezione~\cref{itm:eFXobservable}, si è deciso di fare ampio uso del pattern \engEmph{Observer}, ciascun effetto è progettato per implementare proprietà osservabili (UML in \figurename~\cref{fig:props}), che possono implementare ascoltatori di eventi dedicati in modo semplice, potendosi avvalere delle API messe a disposizione dal framework JavaFX.

            \begin{figure}[htbp]
                \centering
                \includegraphics[scale=0.5]{img/EffectAndPropertiesUML}
        ...
        \subsection{Nodi grafici come monitor per la simulazione}
            ...
            % TODO potrebbe valere la pena aggiungere UML ?
